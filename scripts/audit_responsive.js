/**
 * Адаптивный аудит: 6 страниц × 23 разрешения.
...
const PAGES = [
  ...
  ['/404.html', '404'],
  ['/otdelka-fasadov.html', 'Фасады'],
  ['/uborka-snega-s-kryshi.html', 'Снег'],
];
...
(async () => {
  ...
  for (const [path, pname] of PAGES) {
    await page.goto('http://localhost:8766' + path, { waitUntil: 'networkidle', timeout: 25000 });
